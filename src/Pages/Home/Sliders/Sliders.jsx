...
const Sliders = () => {
   ...
   return (
      <>
         {/* bg-gradient-to-bl from-purple-50 via-transparent to-purple-200 */}
         <div
            className={`py-16
            ${(theme === 'dark' || theme === 'default-dark') ? 'bg-gradient-to-bl from-purple-400 via-purple-50 to-transparent' : ''}
            ${(theme === 'light' || theme === 'default-light') ? 'bg-gradient-to-bl from-purple-400 via-purple-50 to-transparent' : ''}
            ${theme === 'warm' ? 'bg-gradient-to-bl from-purple-400 via-purple-50 to-transparent' : ''}`}
         >
            <div className="relative max-w-7xl mx-auto overflow-hidden rounded glide-06">
                        ...
                        <li key={i} className="relative">
                           <div className="relative"> {/* Container for image & content */}
                              <img
                                 ...
                                 alt={`Slide ${i + 1}`}
                              />
                              <div className="absolute bottom-1/2 translate-y-1/2 left-1/2 transform -translate-x-1/2 w-5/6 md:w-3/6 bg-gray-900 bg-opacity-50 p-4"> {/* Overlay for text & button */}
                                 <div className="flex flex-col gap-4"> {/* Text & button container */}
                                    ...
                                    <button className="px-4 py-2 border border-sky-500 text-white rounded-md">Get in touch</button>
                                 </div>
                              </div>
                           </div>
                        </li>
               ...
               <div className="absolute left-0 flex items-center justify-between w-full h-0 px-4 top-1/2" data-glide-el="controls">
                  <button
                     className="inline-flex items-center justify-center w-8 h-8 transition duration-300 border rounded-full border-sky-500 bg-white/20 text-sky-500 hover:border-slate-900 hover:text-slate-900 focus-visible:outline-none lg:h-12 lg:w-12"
                     data-glide-dir="<"
